...
func main() {
	...
	catImageServer := Server{}
	grpcServer := grpc.NewServer()
	log.Printf("Service created.")

	catimage.RegisterCatImageServiceServer(grpcServer, &catImageServer)
...
func (s *Server) Get(ctx context.Context, _ *catimage.CatImageRequest) (*catimage.CatImageResponse, error) {
	...
	log.Printf("sending image . . . %s", randomFact)

	return &catimage.CatImageResponse{Fact: randomFact}, nil
}
